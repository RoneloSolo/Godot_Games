...
public class Gun : Node2D{
	...
	public override void _Ready(){
		...
		gameManager = GetTree().CurrentScene.GetNode<GameManager>("Managers/GameManager");
	}
	...
	public virtual void Fire(float time, Vector2 mousePos, int projactileAmount = 1, float spread = 0){
		...
		for(int i = 0; i < projactileAmount; i++){
			...
			bull.Start(point.GlobalPosition, point.GlobalRotation + (float)GD.RandRange(-spread,spread),mousePos);
